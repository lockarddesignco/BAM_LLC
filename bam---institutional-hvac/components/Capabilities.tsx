  ...
  return (
    <section id="services" className="relative py-24 bg-[#F7F9F8]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 lg:px-24">
        {/* Section Title */}
        <h2 className="text-center text-2xl md:text-3xl font-semibold text-[#5E7A86] mb-12 tracking-tight">
          Specialized HVAC Services for Mission-Critical Facilities
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div 
              key={index}
              className="bg-white border border-[#D7DDDA] rounded-lg p-6 transition-all duration-300 group h-full flex flex-col cursor-pointer hover:border-[#A7D7C5] hover:-translate-y-1 hover:shadow-[0_4px_12px_rgba(0,0,0,0.04)]"
            >
              <div className="mb-5 text-[#265A4A] group-hover:text-[#5E7A86] transition-colors flex justify-center">
                <service.icon 
                  strokeWidth={1.5} 
                  className="w-10 h-10 transform transition-transform duration-300 group-hover:scale-110" 
                />
              </div>
              <h3 className="text-lg font-bold text-[#265A4A] mb-3">
                {service.title}
              </h3>
              ...
              {/* Learn More Link */}
              <div className="mt-6 flex items-center text-[#265A4A] font-semibold text-sm">
                <span className="group-hover:underline decoration-[#A7D7C5] underline-offset-4 decoration-2">Learn More</span>
        ...
        <div className="mt-16 w-full flex flex-col sm:flex-row items-center justify-center gap-6 pt-16 border-t border-[#D7DDDA]/50">
           <Button variant="primary" size="lg" className="shadow-lg shadow-[#265A4A]/10">
